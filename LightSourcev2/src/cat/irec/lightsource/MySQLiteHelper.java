...
import cat.irec.lightsource.Device;
...
public class MySQLiteHelper extends SQLiteOpenHelper {
	...
	public void onCreate(SQLiteDatabase db) { // SQL statement to create table
		String CREATE_TABLE = "CREATE TABLE devices ( "	+ "id INTEGER PRIMARY KEY AUTOINCREMENT, " + "name TEXT, " + "ip TEXT, " + "port INTEGER, " + "type INTEGER, " + "enable INTEGER )";
	...
	public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
		db.execSQL("DROP TABLE IF EXISTS devices"); // Drop older devices table if existed
		this.onCreate(db); // create fresh devices table
	}

	// -------------------------------------------------------------------------------------------
	// Operations (create "add", read "get", update, delete) device + get all + delete all devices
	// -------------------------------------------------------------------------------------------	 
	...
	private static final String[] COLUMNS = { KEY_ID, KEY_NAME, KEY_IP,	KEY_PORT, KEY_TYPE, KEY_ENABLE };
	...
	public void addDevice(Device device) {
		...
		// 3. insert
		db.insert(TABLE_NAME, /* table */null, /* nullColumnHack */values); // key/value -> keys = column names / values = column values
		// 4. close
		db.close();
	}
	...
	public List<Device> getAllLuminaries() {
		List<Device> devices = new LinkedList<Device>();
		// 1. build the query
		String query = "SELECT * FROM " + TABLE_NAME + " WHERE " + KEY_TYPE + " = 0";
		// 2. get reference to writable DB
		SQLiteDatabase db = this.getWritableDatabase();
		Cursor cursor = db.rawQuery(query, null);
		// 3. go over each row, build device and add it to list
	...
	public int updateDevice(Device device) {
		...
		values.put(KEY_ENABLE, device.getEnable()); // get enable
		// 3. updating row
		int i = db.update(TABLE_NAME, /* table */values, /* column/value */	KEY_ID + " = ?", /* selections */ new String[] { String.valueOf(device.getId()) }); // selection args
		// 4. close
		db.close();
		return i;
	...
	public void deleteDevice(Device device) {
		...
		SQLiteDatabase db = this.getWritableDatabase();
		// 2. delete
		db.delete(TABLE_NAME, KEY_ID + " = ?", new String[] { String.valueOf(device.getId()) });
		// 3. close
		db.close();
